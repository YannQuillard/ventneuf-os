...
Set an absolute `VENTNEUF_CLAUDE_PATH`, add `"claudeDevelopment": true`, and configure an explicit allowlist such as `"claudeModels": ["opus"]` for each repository where Hermes may dispatch Claude Code. Supported aliases are `opus`, `sonnet`, and `fable`. Every Claude mission must select one advertised model; the control plane persists and authorizes that selection, and the runner passes it to Claude with `--model` instead of using the local default. Orca, Claude Code 2.1.261 or newer authenticated as the LaunchAgent user, the local repository registration, and the per-repository opt-in remain required. Claude uses the same isolated worktree lifecycle, two-hour authority, cloud lease, cancellation fencing, clean-worktree removal, and retained diagnostics as Codex. This capability uses the existing mission schema and requires no database migration after `0004`.
