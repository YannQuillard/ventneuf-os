...
Add `"codexDevelopment": true` only to repositories where Hermes may dispatch writable Codex work. The runner advertises this separately from `orcaReview`; the control plane checks the repository owner and capability again before queuing the mission. Add an optional explicit model list such as `"codexModels": ["gpt-5.3-codex"]` when the subscription exposes selectable models. Without that list, Codex uses the subscription default. Development authority lasts at most two hours and is created by the control plane, never by the agent or runner.
...
Set an absolute `VENTNEUF_CLAUDE_PATH`, add `"claudeDevelopment": true`, and configure an explicit allowlist such as `"claudeModels": ["opus"]` for each repository where Hermes may dispatch Claude Code. Supported aliases are `opus`, `sonnet`, and `fable`. Every Claude mission selects an advertised lead model and reasoning effort plus its native subagent models and effort. The control plane persists and authorizes those selections; the runner passes the lead settings with `--model` and `--effort` and defines the selected workers through Claude Code's native `--agents` configuration. Orca, Claude Code 2.1.261 or newer authenticated as the LaunchAgent user, the local repository registration, and the per-repository opt-in remain required. Claude uses the same isolated worktree lifecycle, two-hour authority, cloud lease, cancellation fencing, clean-worktree removal, and retained diagnostics as Codex. This capability uses the existing mission schema and requires no database migration after `0004`.
...
Hermes coordinates and authorizes dispatch only. Mission lead and subagent model selections always belong to the selected Codex or Claude Code subscription and are never applied to Hermes itself.
