...
An enrolled local runner can also register additional repositories from the **Devices** page. Enter a display name and absolute local path; the runner validates and stores the path in its local configuration, while only the generated repository ID and display name are sent to the control plane. Newly registered repositories become available in the project multi-selector after the next runner sync. Use the runner's **Execution harnesses** action to enable Codex or Claude Code once and choose the model IDs exposed by the local subscription. Each mission then selects its harness and models explicitly. The runner writes this configuration to its private local settings and synchronizes the public capability metadata. A harness can be enabled only when its executable was configured during runner installation.
...
Codex is configured once for the runner, independently of its registered repositories. An optional explicit model list such as `"models": ["gpt-5.3-codex"]` exposes selectable models; without that list, Codex uses the subscription default. Every mission selects its harness, lead model and reasoning effort, plus its native sub-agent plan. The control plane still checks the initiating member, project association, device and repository before queuing work. Development authority lasts at most two hours and is created by the control plane, never by the agent or runner.

The runner creates an isolated Orca worktree with setup hooks skipped and starts Codex through its App Server protocol in a visible owned terminal. Codex uses its standard `workspace-write` sandbox and native `on-request` approval policy. The runner does not install a second filesystem profile, command grammar, executable allowlist, runtime wrapper, or temporary-directory policy. Repository instructions and the coding agent's normal tools remain active.

Development missions keep live hosted web search, installed skill discovery and search, repository image inspection, image generation, and bounded subagent delegation available. A project mission selects the Codex lead model and reasoning effort plus the requested native subagent models and effort. The runner sends the lead settings through the App Server protocol and includes the subagent plan in the mission instructions; Codex performs delegation through its own multi-agent capability. The runner does not disable native apps, plugins, browser tools, hooks, shell snapshots, dependency workflows, or other supported Codex capabilities. Web and connector results remain untrusted, search queries must not contain secrets or private repository content, and every external effect remains subject to the mission's authority. Subagents inherit the same mission and worktree boundary.

Codex works through a clean pushed branch and pull request. Native command, file-change, and permission requests are converted into durable operations with a SHA-256 argument digest and a persisted Codex session reference. Network access, pull-request creation, and broader permissions go to Hermes; pull-request merges and deployment applies require the initiating member. The runner rejects attempts to move the active command or direct file-change root outside the mission worktree. Review evidence contains only a bounded operation label and validated network destination; command arguments stay local while the digest remains bound to the original request.
...
Set an absolute `VENTNEUF_CLAUDE_PATH` and configure the Claude Code models once on the runner, for example `{"claude":{"models":["opus"]}}`. Supported aliases are `opus`, `sonnet`, and `fable`. Every Claude mission selects an advertised lead model and reasoning effort plus its native subagent models and effort. The control plane persists and authorizes those mission selections; the runner passes the lead settings with `--model` and `--effort` and defines the selected workers through Claude Code's native `--agents` configuration. Orca, Claude Code 2.1.261 or newer authenticated as the LaunchAgent user, and a mission-authorized local repository registration remain required. Claude uses the same isolated worktree lifecycle, two-hour authority, cloud lease, cancellation fencing, clean-worktree removal, and retained diagnostics as Codex.

Claude runs in native automatic permission mode with its standard tools and sandbox required. Repository writes include the mission worktree and the linked Git paths needed for commits. The runner does not filter programming languages, executables, package managers, multiline commands, or shell composition. Credential-valued environment variables remain unavailable to sandboxed commands. Hosted web search and fetch, installed skills, workflows, and bounded subagents remain available for autonomous research and implementation. The strict MCP set stays empty until mission-scoped connectors are injected through ventneuf MCP.

Native `PreToolUse` hooks defer a concrete elevated or external operation and persist the Claude session and tool identity. Hermes reviews a bounded description and SHA-256 argument digest while the raw tool input stays on the runner. Approved native sandbox escapes resume only that exact request. For Git pushes and pull-request creation, the supervisor brokers the required authority and credentials, then reports the outcome back to the same Claude session. Merge and deployment remain explicit control-plane authority decisions.
