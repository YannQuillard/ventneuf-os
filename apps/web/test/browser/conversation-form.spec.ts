...
async function openForm(page: Page, options: { otherMember?: boolean; failFirst?: boolean } = {}) {
  ...
  await page.route("**/api/workspace", route => route.fulfill({ json: {
    currentMember: { id: options.otherMember ? "other-member" : memberId, name: "Member" }, members: [], projects: [],
    conversations: [{ id: conversationId, title: "Performance", kind: "private", ownerMemberId: memberId, recipients: [], canManage: true, isOwner: true, createdAt, updatedAt: createdAt }],
  } }));
