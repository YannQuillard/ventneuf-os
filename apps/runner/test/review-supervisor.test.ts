...
test("independent supervisor kills its owned process when confirmed lease updates stop", { timeout: 8_000 }, async () => {
  ...
  try {
    ...
    await writeFile(executable, `#!${process.execPath}\n`
      ...
      + `require("node:fs").writeFileSync(${JSON.stringify(started)}, JSON.stringify({ pid: process.pid, environment: Object.keys(process.env) }));\n`
      + "process.on('SIGTERM', () => {}); setInterval(() => {}, 100);\n", { mode: 0o700 });
    await writeReviewState(join(directory, "job.json"), { codexPath: executable, snapshot, deadline: Date.now() + 30_000 });
    ...
    const child = JSON.parse(await readFile(started, "utf8")) as { pid: number; environment: string[] };
